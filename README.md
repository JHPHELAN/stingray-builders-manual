...
best-known state as of 2026-07-20.
...
Enclosure:  Cast acrylic top and side plates, laser-cut
            at FabLab.  Full mechanical CAD in
            CAD files/ subdirectory of the workspace.

Note on the Pi 5 history: Stormy is on the SECOND Pi 5.
The first had a broken GPIO 14 TX pin, diagnosed on
2025.03.17 with a minicom loopback test.  Replacement was
ordered from Amazon and works fine.  See stardate 2025.03.17
in the source log for the diagnostic method.
...
Physical mount:  At base_link - squarely between the two
    ...
    and motor wiring is irrelevant to this IMU.  Sergei
    Grichine's "place IMU far from motors" rule applied to
    the earlier BNO055 (which used its magnetometer and did
    suffer field noise); it does not apply to Stormy's
    mag-disabled BNO085.
...
Full contents (canonical):
    ...
    bno08x_driver:
      ros__parameters:
        ...
        # Use game-rotation-vector mode (no magnetometer
        # for absolute yaw).  Indoor use only.  See
        # Field Note item 42 for the top-level-vs-nested
        # YAML gotcha.
        imu:
...
Rationale for `use_magnetometer: false`:  indoor magnetic
distortion (motor magnets, current-carrying bus wiring,
steel in nearby structures) makes absolute mag-north yaw
unstable.  Game-rotation-vector uses gyro + accel only.
Absolute yaw drifts slowly but is stable in the short term,
which is what SLAM correction handles.
...
Purpose:   Detect obstacles BELOW the LiDAR plane
           ...
           shoe piles, and the dog.

In nav2_params.yaml, both local_costmap and global_costmap:

    obstacle_layer:
      plugin: "nav2_costmap_2d::ObstacleLayer"
      enabled: true
      observation_sources: laser oakd
      laser:
        data_type: "LaserScan"
        topic: /scan
      ...
      oakd:
        ...
        topic: /oak/points
        min_height: 0.02       # skip floor/rugs
        max_height: 0.31       # top of USB WiFi antenna
                               # (was 0.22, just below
                               # LiDAR - raised so tall
                               # obstacles that clear the
                               # LiDAR plane still mark)
        expected_update_rate: 10.0
        marking: true
        clearing: false        # don't clear on OAK-D
                                # dropouts
...
### 9.1  Interfaces (current, 2026-07-20)
    ...
    eth0                    Wired Ethernet, 192.168.68.97
    ...
    wlan0                   Pi 5 internal WiFi.  DISABLED
                            ...
                            by design).
    wlx90de801012a6         Realtek RTL8812BU USB WiFi
                            dongle on the powered USB hub.
                            192.168.68.66, DHCP-reserved.
...
to leak to wlan0.  Cleanest solution is to keep wlan0 down.
    ...
    # SSH in over USB dongle IP 192.168.68.66 first!
    sudo nmcli con modify netplan-wlan0-H0bb1tH0l3 \
                          connection.autoconnect no
    sudo nmcli dev disconnect wlan0

    # Verify:
    hostname -I
    # Should show ONLY 192.168.68.66 (and possibly eth0's
    # 192.168.68.97 when docked)

To re-enable if USB dongle fails:
    sudo nmcli con modify netplan-wlan0-H0bb1tH0l3 \
                          connection.autoconnect yes
    sudo nmcli con up netplan-wlan0-H0bb1tH0l3

### 9.3  TP-Link Deco mesh layout


Three Deco units on ethernet backhaul.  All same model
(OUI 34:60:F9).  Radio BSSIDs = sticker MAC + 2 (2.4 GHz)
and + 3 (5 GHz):

    Location    Base MAC              2.4 GHz    5 GHz
    Study       34:60:F9:5D:7C:14     :7C:16     :7C:17
    Sewing      34:60:F9:5D:7B:C4     :7B:C6     :7B:C7
    Breakfast   34:60:F9:5D:7C:B0     :7C:B2     :7C:B3

Breakfast unit moved from Master Closet on 2026.07.10 for
better far-end coverage.
...
Configure git:
    git config --global user.name "James H Phelan"
    git config --global user.email "jhphelan@hal-pc.org"
...
Tuning that handles Stormy's kitchen-island feature-poor
area (Field Note item 64):
...
Inputs:
    StormyHouseMapWallsOnly.jpg     CorelDraw export of
                                    walls-only layer;
                                    2936 x 1834 CMYK
    StormyHouseMapXMarksDock.jpg    Same base image with
                                    a green cross at the
                                    robot dock location

CorelDraw scale factor:
    0.73 in in drawing = 72 in in life
    drawing_dist = life_dist * (0.73 / 72)

Blueprint image is 2936 px wide for the 81 ft house width:
    24.69 m / 2936 px = 0.00841 m / px (source resolution)

Target Nav2 resolution: 0.05 m/px (matches all other maps
on Stormy).  Output PGM is downscaled to ~494 x 309 px via
`cv2.INTER_AREA` (preserves thin walls).

Dock origin calibration:
    Green cross at pixel (2500, 955) in source image.
    x_origin = -2500 * 0.00841           = -21.02 m
    y_origin = -(1834 - 955) * 0.00841   =  -7.39 m
...
walls.  Stormy's final:
    origin: [-3.407, -8.114, 0.0]

Door handling:
    All interior doors passable EXCEPT bedroom 2 and
    bedroom 3 closet doors (closed by painting wall pixels
    across the doorway post-flood-fill).
...
Bash aliases assumed (Chapter 19).
    ...
    Terminal 2:  stingSLAM   # bringup in mapping mode
...
WAV files live in `~/wav/`.  Notable clips:
    klingon_alert.wav          Beacon
    danger.wav                 "Danger Will Robinson" -
                               Nav2 abort / recovery
    call-to-post.wav           stingln + stingSLAM startup
    forcefield_powering_down.wav  scram (shutdown) alias
    tos_alien_sound_2.wav      misc
...
Stingray (Pi 5 on the robot):
    Hostname:      Stingray
    IPs:           192.168.68.97 (eth0)
                   192.168.68.66 (wlxUSB)
...
LinuxBox (development laptop, wired or WiFi):
    Hostname:      LinuxBox
    IPs:           192.168.68.99 (enp3s0f1 ethernet)
                   192.168.68.98 (wlp2s0 WiFi; usually
...
Terminal 1 (Stingray, LAUNCH panel):
    stingln       # OR stingSLAM for mapping mode
    ...
    <CycloneDDS>
      <Domain>
        ...
        <Discovery>
          ...
          <Peers>
            <Peer address="192.168.68.66"/>   <!-- Stingray -->
            <Peer address="192.168.68.99"/>   <!-- LinuxBox -->
          </Peers>
        </Discovery>
      </Domain>
    </CycloneDDS>
...
canonical set on Stingray:
    ...
    alias stingln='aplay -q ~/wav/call-to-post.wav; \
                   ros2 launch articubot_one stingray.launch.py'
    alias stingSLAM='aplay -q ~/wav/call-to-post.wav; \
                     ros2 launch articubot_one stingray.launch.py slam:=true'
    ...
    alias explore='ros2 launch articubot_one explore.launch.py'
    ...
    alias anchor1='...'    # study door (1.32, 1.32, pi/4)
    ...
    # Shutdown
    alias scram='aplay -q ~/wav/forcefield_powering_down.wav; \
                 sudo shutdown -h now'
...
Placeholder note: `<mount>` in the commands below is the
mount-point directory name that GNOME assigns your external
USB backup drive under /media/ubuntu/ (the filesystem label
if the drive has one, otherwise its UUID).  Find yours with:

    lsblk -o NAME,SIZE,LABEL,MOUNTPOINT
    ls /media/ubuntu/

Example real values seen on Stormy: `BACKUP2026`,
`Stormy_Backups`, `1A2B-3C4D`.  Substitute yours everywhere
`<mount>` appears (Setup, Backup, Verify).

Setup - one-time drive format (external USB drive, all data
lost):

    # Unmount any auto-mounted partition first:
    sudo umount /media/ubuntu/<mount>
    # Wipe partition table:
    sudo parted /dev/sda mklabel gpt
    sudo parted /dev/sda mkpart primary 0% 100%
    sudo partprobe /dev/sda
    # exFAT is Windows AND Linux readable:
    sudo apt install exfatprogs
    sudo mkfs.exfat /dev/sda1

Backup - the working command (note `sudo sh -c` wraps the
whole pipeline so the redirection runs as root - Field
Note item 39):

    sudo sh -c 'dd if=/dev/nvme0n1p2 bs=4M \
                    conv=sync,noerror status=progress | gzip -1 \
                    > /media/ubuntu/<mount>/nvme_root.img.gz'

    # ~2250 s (~38 min) at ~115 MB/s for 238 GB
    # ~13 GB compressed for a fresh Ubuntu install

    # ALSO back up the boot partition (500 MB, ~2 min):
    sudo sh -c 'dd if=/dev/nvme0n1p1 bs=4M \
                    conv=sync,noerror status=progress | gzip -1 \
                    > /media/ubuntu/<mount>/nvme_boot.img.gz'

Verify integrity:
    gzip -t /media/ubuntu/<mount>/nvme_root.img.gz && echo OK
    gzip -t /media/ubuntu/<mount>/nvme_boot.img.gz && echo OK

Restore (from a booted rescue system):
    gunzip -c /path/to/nvme_root.img.gz \
        | sudo dd of=/dev/nvme0n1p2 bs=4M status=progress
    gunzip -c /path/to/nvme_boot.img.gz \
        | sudo dd of=/dev/nvme0n1p1 bs=4M status=progress

USB drive MUST be on a powered hub OR directly on a blue
USB 3.0 port for the ~40 min copy (Field Note item 86).
Bus-powered drives disconnect mid-copy.
...
Step 6 - Verify hardware:
    ...
    ros2 launch depthai_ros_driver_v3 camera.launch.py
...
Step 8 - Full launch dry-run (wheels off ground):
    joy
    stingln
    # Verify joystick moves motors, /odom updates,
    # /joint_states publishes, /scan appears.

Step 9 - Restore map assets:
    Already in the articubot_one repo under
    `assets/maps/`.  Verify:
        ls ~/robot_ws/src/articubot_one/assets/maps/
        # Should include Stormy_blueprint.pgm/yaml
        # and Stormy_merged.pgm/yaml.
...
# WiFi
iwconfig wlan0
iwconfig wlx90de801012a6
iw dev wlan0 link
iw dev wlx90de801012a6 link
...
Stormy tuning (holds localization in kitchen-island
feature-poor area, Field Note item 64):
    max_particles:              5000
    min_particles:              1000
    laser_max_beams:             120
    laser_likelihood_max_dist:   4.0
...
stingSLAM                             # bringup in mapping mode
...
Version 1.0.  Living document.  Amend as Stormy evolves.
