...
Tier 4a workflow (monthly full image):
    ...
    month, while the HDD is still plugged in.

    1. On Hank Rearden, FileZilla to Stormy and drag the two
       most recent files
           /mnt/backup/nvme_boot_YYYY-MM-DD.img.gz
           /mnt/backup/nvme_root_YYYY-MM-DD.img.gz
       into
    ...
    3. On Stormy, tell backup_check.sh that a copy just
       happened so the "days since Tier 4a" counter resets:
           mkdir -p ~/.stormy && touch ~/.stormy/last_icloud_copy
    4. In %USERPROFILE%\iCloudDrive\Stormy\, delete anything
       older than the previous month's pair.  Keeps two
       generations rolling.
...
Tier 4b workflow (weekly rescue kit):
    ...
    2. FileZilla from Hank Rearden, drag the .tar.gz into
       %USERPROFILE%\Dropbox\Stormy\.
    3. Dropbox syncs in the background.
    4. Dropbox-side pruning runs automatically each week
